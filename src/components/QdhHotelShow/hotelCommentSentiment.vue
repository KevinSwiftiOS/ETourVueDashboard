<template>
    ...
    >
        <div id="wordcloud">
            <block-box :title="'酒店有关'+paramList.tagname+'评论分析'" icon="icon-comment">
                <el-row>
...
<script>
    ...
    import commentAnalyze from "../../components/QdhHotelShow/qdhHotelCommentAnalyze"
    import PageChange from '../../components/divide-page/PageChange'
    import {remove_session_storage} from "../../base/sessionStorage";
    ...
    export default {
        ...
        methods: {
            ...
            getComTagSum: async function () {
                this.loading = true;
                const res = await http.hotelComTagSum({});
                this.loading = false;
                if (res.code === 0 && res.data.infoList) {
                    this.storeClassTagNum(res.data.infoList)
                    console.log(res.data.infoList, '输出这个矩阵');
            ...
            typeChangeBtn: async function (choiceType, val) {
                ...
                this.changeCommentClass();
                const res = await http.hotelComFeatureWord(this.paramList);
