...
<script>
    ...
    import {get_color_hotel,get_radius_hotel} from "../base/getColorForHotel";
    ...
    export default {
      ...
      <div class="map-label">
        <div class="header">
            ...
            <span class="comment">
              <span class="score"><i>${obj.commentScore.toFixed(2)}</i>/5.0</span>
...
// 继承API的BMap.Overlay
            initMap: async function() {
                //初始化MAP
                this.map = new BMap.Map("map", { minZoom: 13, maxZoom: 15 });
            ...
            loadmarkers() {
                ...
                // }
                map.clearOverlays();  // 清除之前所有覆盖物，也可以研究一下怎样清楚具体坐标
                // 筛选marker
                let data = [];
                if (this.value === "") {
                    data = this.data;
...
// 根据地理坐标转换为像素坐标，并设置给容器
                    ...
                    var width = 15 + get_radius_hotel(item.commentNumber, this.isHome); // 这里是返回的值 再加上 前面的数
                    ...
                    if (this.isHome)
                        ...
                        title = item.name;
                    var mySquare = new SquareOverlay(point, width, get_color_hotel(item.commentScore, this.isHome), title);
        ...
        watch: {
            data(val) {
                console.log(val.length, '输出酒店的长度');
                this.loadmarkers();
            },
            value(newVal) {
                this.loadmarkers();
