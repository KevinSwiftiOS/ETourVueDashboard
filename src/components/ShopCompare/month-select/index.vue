  ...
  <el-row :gutter="20" class="month-search">
    ...
    <!-- <el-col :xs="24" :sm="24" :md="4" class="item">
      <el-select v-model="website" style="width:100%">
        <el-option v-for="item in websites" :key="item" :label="item" :value="item">
        </el-option>
      </el-select>
    </el-col> -->

    <el-col :xs="24" :sm="24" :md="10" class="item">
      <!--  -->
      <el-select
        v-model="searchData.selectBusinessCirle"
        multiple
        placeholder="请选择商圈"
        style="width:100%"
      >
        <el-option
          v-for="item in total_areas"
          :key="item"
          :label="item"
          :value="item"
        >
...
import { total_areas, cuisine } from "../../../base/dict";
...
export default {
  ...
  methods: {
    getWeek(date) {
      ...
      const year = momentData.weekYear();
    ...
    search() {
      ...
      if (this.searchData.time === "周") {
        ...
        // this.searchData.startTime = moment(this.week1).format('YYYY-MM-DD');
        // this.searchData.endTime = moment(this.week2).format('YYYY-MM-DD');
      ...
      if (this.searchData.selectBusinessCirle.length == 0) {
        this.$message.warning("请选择商圈进行对比");
        return;
      }
