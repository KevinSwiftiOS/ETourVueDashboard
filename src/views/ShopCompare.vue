                                ...
                                <el-row :sm='24' :md='24'>
                                    <el-col :sm="2" :md="2">
                                        <template v-for="item in feature">
                                            <el-badge :type="item.type" :value="item.count" :key="item.name"
                                                      class="item1">
                                                <el-button class="btn" :type="item.type" size="small"
                                                           @click="showComments(item.key)" plain>{{item.name}}
                                                </el-button>
                                            </el-badge>
                                        </template>
                                    </el-col>
                                    <el-col :sm="22" :md="22">
                                        <div class="block">
                                            <template v-for="item in button">
                                                <el-badge :type="item.type" :value="item.count" :key="item.name"
                                                          class="item2">
                                                    <el-button class="btn2" :type="item.type" size="small"
                                                               @click="clickButton(item.key)" plain>{{item.name}}
                                                    </el-button>
                                                </el-badge>
                                            </template>
                                            <el-carousel class="card" indicator-position="none" trigger="click"
                                                         height="500px" :autoplay="autoplay" :interval="interval"
                                                         v-on:change="changeFun" arrow="hover">
    ...
    export default {
        ...
        data() {
            return {
                ...
                featureWordArray: { // 特征词
                    server: ["服务", "态度"],
                    taste: ["好吃", "新鲜", "份量大", "鲜美", "口感棒", "口味", "难吃", "味道"],
                    evn: ["干净", "整洁", "方便", "卫生", "乱", "脏", "差", "精致", "热闹", "恶心", "不可能", "装修", "人气"],
                    price: ["实惠", "推荐", "满意", "排名", "口碑", "性价比", "强推", "差评", "失望", "不够吃", "贵", "不会去"]
                },
                button: [
        ...
        methods: {
            ...
            showDate: function (val) {
                ...
                var newStr = [];
                var Str = '';
                for (var i = 0; i < temp.length; i++) { // 一句话的数组
                    for (var j = 0; j < arr.length; j++) {
                        if (temp[i].indexOf(arr[j]) != -1) {
                            temp[i] = temp[i].replace(temp[i], '<font color="#409EFF">' + temp[i] + '</font>')
                            break;
                        }
                    }
                }
            ...
            searchchart1: async function ({startTime, endTime, time, selectBusinessCirle}) {
                this.loading = true;
            ...
            searchchart2: async function ({startTime, endTime, time, selectBusinessCirle}) {
                this.loading = true;
